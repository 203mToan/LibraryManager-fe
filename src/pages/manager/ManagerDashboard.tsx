...
export default function ManagerDashboard() {
  const totalBooks = mockBooks.reduce((sum, book) => sum + book.quantity, 0);
  // const availableBooks = mockBooks.reduce((sum, book) => sum + book.available, 0);
  const totalBorrowers = mockUsers.filter((u) => u.role === 'borrower').length;
  const activeLoans = mockLoans.filter(
    (l) => l.status === 'approved' || l.status === 'overdue'
  ).length;
  // const totalFines = mockLoans.reduce((sum, loan) => sum + loan.fine, 0);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          ...
          <div className="space-y-3">
            {mockBooks.slice(0, 5).map((book, index) => (
              <div
                key={book.id}
                className="flex items-center justify-between p-3 bg-gray-50 rounded-lg"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      {book.quantity - book.available} đã được mượn
                    </p>
                  </div>
                </div>
